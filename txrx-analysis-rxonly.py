...
def plot_statistics(df):
    ...
    df['time_tx'] = pd.to_datetime(df['frame.time_epoch'], unit='s')
    # Normalize the timestamps to start from 0
    df['time_from_start(s)'] = (df['time_tx'] - df['time_tx'].iloc[0]).dt.total_seconds()
    ...
    columns_to_plot = [column for column in df.columns if column not
                       in ['time_tx', 'iperf.id', 'lost', 'out-of-order']]
    ...
    # sns.scatterplot(data=df, x='iperf.id', y= 'frame.time_epoch')  #, hue='Flows')
